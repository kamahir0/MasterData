...
そのため、本体は UPM パッケージというより、モノレポ内の `master-data` プロジェクトに配置する外部ツールに近い。
...
- `init` の引数省略時は標準名 `master-data/` を作成する
- 既存の project root を直接初期化したい場合は `.` を明示する
- converter は `converter/` 配下に置き、project root を相対指定できるようにする

ただし最終的には Tauri エディタも存在するため、CLI の init/convert と editor の project open/init は役割分担になる。

## sync と monorepo

想定ユースケースは、Unity client と master-data が同じモノレポにある構成。
...
- master-data project 単体で完結した設定にしやすい
...
Unity6.4Sandbox 側に `Assets/MasterDataTest/master-data~` を作り、実ユースケースに沿って動作確認した。
...
- MasterData は Unity runtime package というより外部 master-data tool
