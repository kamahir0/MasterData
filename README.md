...
master-data project 用のディレクトリを作成し、その直下に対象 platform の init
バイナリを置いて実行します。引数を省略した場合は、init バイナリが置かれている
ディレクトリを project root として初期化します。初期化後、`MasterDataInit-*` は
削除してかまいません。

```bash
mkdir my-master-data
cd my-master-data
chmod +x ./MasterDataInit-osx-arm64
xattr -d com.apple.quarantine ./MasterDataInit-osx-arm64
./MasterDataInit-osx-arm64
rm ./MasterDataInit-osx-arm64
```

別のディレクトリから初期化したい場合は、対象ディレクトリを明示します。
相対パスは現在のディレクトリから解決されます。現在のディレクトリ自体を
project root にしたい場合は `.` を指定します。

```bash
./MasterDataInit-osx-arm64 .
./MasterDataInit-osx-arm64 ./my-master-data
./MasterDataInit-osx-arm64 ./my-master-data --force
./MasterDataInit-osx-arm64 ./my-master-data --no-download
```
...
`MasterDataInit` は最初に解決した project root を表示します。ターミナルから実行された
場合は、対話形式で C# namespace、YAML 入力ディレクトリ、ローカル出力先、Unity
同期先を質問します。非対話環境または `--yes` 指定時はデフォルト値を使います。
